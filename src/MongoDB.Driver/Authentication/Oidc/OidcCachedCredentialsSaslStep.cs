...
{
    internal sealed class OidcCachedCredentialsSaslStep : OidcSaslStep, ISaslStep
    {
        private readonly OidcCredentials _credentials;

        public OidcCachedCredentialsSaslStep(OidcCredentials credentials)
        {
            _credentials = Ensure.IsNotNull(credentials, nameof(credentials));
        }
        ...
        public (byte[] BytesToSendToServer, ISaslStep NextStep) Execute(
            ...
            => PreparePayload(_credentials);

        public Task<(byte[] BytesToSendToServer, ISaslStep NextStep)> ExecuteAsync(
            SaslConversation conversation,
            byte[] bytesReceivedFromServer,
            CancellationToken cancellationToken)
            => Task.FromResult(PreparePayload(_credentials));
    }
}
