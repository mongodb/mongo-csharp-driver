...
GridFS JSON spec tests run through `UnifiedTestSpecRunner.GridFS` (`[UnifiedTestsTheory("gridfs.tests")]`), which picks up everything under `specifications/gridfs/tests/` automatically, and the spec's prose tests live in `tests/MongoDB.Driver.Tests/Specifications/gridfs/prose-tests/`. GridFS coverage also lives in the regular xUnit tests under `tests/MongoDB.Driver.Tests/GridFS/`, plus the JSON-driven runners pulled in by other specs: `tests/MongoDB.Driver.Tests/JsonDrivenTests/JsonDrivenGridFs*.cs` and `tests/MongoDB.Driver.Tests/UnifiedTestOperations/UnifiedGridFs*Operation.cs` exercise GridFS as part of the unified / JSON-driven runners used by transactions, retryable reads/writes, etc.

## Common pitfalls

- **Unwrapped file IDs in filters.** Every driver-built filter carrying a user-provided file id must go through `GridFSIdFilter.Create`, which wraps the value in `$eq`. A serialized `TFileId` can itself be a document, whose keys the server would otherwise read as query operators (CSHARP-6190 / CVE-2026-88025). The spec requires this for `_id` on `.files` and `files_id` on `.chunks`.
