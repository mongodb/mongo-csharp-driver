...
namespace MongoDB.Driver.Core.Clusters
{
    internal sealed class ClusterClock : IClusterClock
    {
        ...
        public void AdvanceClusterTime(BsonDocument newClusterTime)
        {
            ...
            BsonDocument currentClusterTime;
            BsonDocument newValue;
            do
            {
                currentClusterTime = _clusterTime;
                newValue = GreaterClusterTime(currentClusterTime, newClusterTime);
            } while (!ReferenceEquals(Interlocked.CompareExchange(ref _clusterTime, newValue, currentClusterTime), currentClusterTime));
        }
    }
}
