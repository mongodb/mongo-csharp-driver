...
{
    internal sealed class ToHashSetBinder : IMethodCallBinder<EmbeddedPipelineBindingContext>
    {
        ...
        public Expression Bind(PipelineExpression pipeline, EmbeddedPipelineBindingContext bindingContext, MethodCallExpression node, IEnumerable<Expression> arguments)
        {
            return new PipelineExpression(
                pipeline.Source,
                pipeline.Projector,
                new HashSetResultOperator(
                    node.Type,
                    SerializerHelper.CreateHashSetSerializer(pipeline.Projector.Serializer)));
        }
    }
}
