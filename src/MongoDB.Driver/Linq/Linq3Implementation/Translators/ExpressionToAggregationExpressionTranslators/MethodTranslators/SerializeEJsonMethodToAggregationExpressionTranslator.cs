...
{
    internal class SerializeEJsonMethodToAggregationExpressionTranslator
    {
        public static TranslatedExpression Translate(TranslationContext context, MethodCallExpression expression)
        {
            ...
            var (relaxedAst, onErrorAst) = TranslateOptions(context, expression, optionsExpression, outputSerializer);

            var ast = AstExpression.SerializeEJson(valueTranslation.Ast, relaxedAst, onErrorAst);
            return new TranslatedExpression(expression, ast, outputSerializer);
        }

        private static (AstExpression relaxedAst, AstExpression onErrorAst)
            TranslateOptions(
        ...
        private static (AstExpression relaxedAst, AstExpression onErrorAst)
            TranslateOptions(
                ...
                IBsonSerializer outputSerializer)
        {
            var options = (SerializeEJsonOptions)optionsExpression.Value;

            AstExpression relaxedAst = null;
            AstExpression onErrorAst = null;
            if (options != null)
            {
                if (options.Relaxed.HasValue)
                {
                    relaxedAst = AstExpression.Constant(options.Relaxed.Value);
                }
                ...
                IBsonSerializer outputSerializer)
        {
            ...
            return (relaxedAst, onErrorTranslation?.Ast);
        }
    }
}
