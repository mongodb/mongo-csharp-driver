...
internal static class TakeLastMethodToAggregationExpressionTranslator
{
    public static TranslatedExpression Translate(TranslationContext context, MethodCallExpression expression)
    {
        ...
        if (method.IsOneOf(EnumerableOrQueryableMethod.TakeLastOverloads))
        {
            ...
            var countAst = AstExpression.Max(countTranslation.Ast, 0); // map negative counts to 0 (TakeLast(<= 0) yields an empty array)
            var negatedCountAst = AstExpression.Subtract(0, countAst); // a negative $slice count takes elements from the end of the array

            var ast = AstExpression.Slice(sourceTranslation.Ast, negatedCountAst);
