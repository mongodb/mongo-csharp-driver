...
namespace MongoDB.Driver.Linq.Linq3Implementation
{
    internal abstract class GroupingWithOutputExpressionStageDefinition<TInput, TGrouping, TOutput> : PipelineStageDefinition<TInput, TOutput>
    {
        ...
        {
            if (renderArgs.LinqProvider != LinqProvider.V3)
            {
            ...
            var serializerRegistry = renderArgs.SerializerRegistry;
        ...
        public override RenderedPipelineStageDefinition<TOutput> Render(RenderArgs<TInput> renderArgs)
        {
            if (renderArgs.LinqProvider == LinqProvider.V2)
            {
                var linq2Stage = PipelineStageDefinitionBuilder.Bucket(
                    ...
                    _options);
                return linq2Stage.Render(renderArgs);
            }
            else
            {
                return base.Render(renderArgs);
            }
        }
