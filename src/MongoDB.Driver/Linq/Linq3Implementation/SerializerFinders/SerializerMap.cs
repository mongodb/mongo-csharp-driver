    ...
    public void AddSerializer(Expression node, IBsonSerializer serializer)
    {
            ...
            serializer is INullableSerializer nullableSerializer)
        {
            if (nodeNonNullableType.IsEnum(out var targetEnumUnderlyingType) && targetEnumUnderlyingType == serializerNonNullableType)
            {
                ...
                var integralTypeSerializer = nullableSerializer.ValueSerializer;
                var convertEnumToIntegralTypeSerializer = ConvertEnumToIntegralTypeSerializer.Create(enumType, integralTypeSerializer);
                serializer = NullableSerializer.Create(convertEnumToIntegralTypeSerializer);
            }
            else if (serializerNonNullableType.IsEnum(out var serializerUnderlyingType) && serializerUnderlyingType == nodeNonNullableType)
            {
                ...
                var convertIntegralTypeToEnumSerializer = ConvertIntegralTypeToEnumSerializer.Create(integralType, enumSerializer);
                serializer = NullableSerializer.Create(convertIntegralTypeToEnumSerializer);
            }
        }
