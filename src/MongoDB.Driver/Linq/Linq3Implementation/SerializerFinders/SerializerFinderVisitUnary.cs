    ...
    protected override Expression VisitUnary(UnaryExpression node)
    {
        ...
        IBsonSerializer GetConvertSerializer()
        {
            ...
            static IBsonSerializer GetTargetSerializer(UnaryExpression node, Type sourceType, Type targetType, IBsonSerializer sourceSerializer)
            {
                ...
                {
                    return GetConvertEnumToIntegralTypeSerializer(node, sourceType, targetType, sourceSerializer);
                }

                if (IsConvertIntegralTypeToEnum(sourceType, targetType))
                {
                    return GetConvertIntegralTypeToEnumSerializer(node, sourceType, targetType, sourceSerializer);
                }
            ...
            static IBsonSerializer GetConvertEnumToIntegralTypeSerializer(UnaryExpression expression, Type sourceType, Type targetType, IBsonSerializer sourceSerializer)
            {
                var enumSerializer = sourceSerializer;
                var integralType = targetType;

                // the serializer converts in the opposite direction as the C# expression
                return ConvertIntegralTypeToEnumSerializer.Create(integralType, enumSerializer);
            }
            ...
            static IBsonSerializer GetConvertIntegralTypeToEnumSerializer(UnaryExpression expression, Type sourceType, Type targetType, IBsonSerializer sourceSerializer)
            {
                IBsonSerializer targetSerializer;
                if (sourceSerializer is IConvertIntegralTypeToEnumSerializer convertIntegralTypeToEnumSerializer)
                {
                    targetSerializer = convertIntegralTypeToEnumSerializer.EnumSerializer;
                }
                else
                {
            ...
            static bool IsConvertEnumToIntegralType(Type sourceType, Type targetType)
            {
                return
                    sourceType.IsEnum &&
