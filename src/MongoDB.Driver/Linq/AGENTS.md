...
- **Constant quoting is centralized in `AstConstantExpression.Render()`.** A constant is wrapped in `$literal` whenever its value contains something the server would reinterpret as an expression — a `$`-prefixed string (field path or variable) or a `$`-prefixed document element name (operator) — at any depth inside documents and arrays. Constants routinely carry user data (`PartialEvaluator` folds closure-captured objects, dictionaries, and arrays into a `ConstantExpression`), so leaving one unquoted lets a `$`-prefixed string read a field the query never asked for. Don't add a second quoting step: `ProjectionHelper.QuoteIfNecessary` deliberately skips values that already quote themselves and covers only the extra `$project`-specific cases (booleans, numbers, plain documents — which the server would otherwise read as include/exclude specifications).
- **Field names cannot be quoted — validate them instead.** `$literal` protects a constant *value*, but an element name is emitted into the pipeline verbatim, so anywhere a field name comes from application data it must be checked with `AstFieldName.IsSafe` (rejects empty, `$`-prefixed and dotted names). Two places take a name from a folded constant: `NewBsonDocumentExpressionToAggregationExpressionTranslator` throws `ExpressionNotSupportedException` for an unsafe name, and `AstSimplifier`'s `$arrayToObject` rewrite only promotes a `k` constant to an element name when it is safe — otherwise it leaves `$arrayToObject` in place, where the key stays data. Adding a new translator that builds an `AstComputedField` from user-supplied text needs the same check.
- **The query language has no `$literal`.** In a `$match`/`Find` filter a document whose first element name starts with `$` *is* a set of operators, and there is no escape. That is why `AstSimplifier` refuses to collapse an `$eq` whose value is such a document down to an implied operation — the explicit `$eq` is what keeps it a value comparison. Both predicates that feed an `AstImpliedOperationFilterOperation` guard against it via `AstImpliedOperationFilterOperation.CanRepresent`, and that node's constructor throws as a last line of defense if a rewrite ever forgets: `IsFieldEqValue` (`{ field : { $eq : value } }` → `{ field : value }`) and `IsFieldElemMatchEqValue` (`{ field : { $elemMatch : { $eq : value } } }` → `{ field : value }`). `IsFieldNotElemMatchEqValue` deliberately does not — it rewrites to an explicit `{ field : { $ne : value } }`, where the value stays an operand and is never read as operators. Don't "simplify" an `$eq` wrapper away, and apply the same check to any new rewrite that promotes a value to an implied operation.
