...
public abstract class InsertManyResult
{
    ...
    /// When the driver did not assign an id (for example when the id was generated by the server, or the document's
    /// serializer does not expose an id) the entry reflects whatever the document carried — typically null, or the id
    /// member's default value (such as 0 for an int id).
    /// </summary>
