...
namespace MongoDB.Driver
{
    ...
    public abstract class PipelineDefinition<TInput, TOutput>
    {
        ...
        /// <returns>A <see cref="RenderedPipelineDefinition{TOutput}"/></returns>
        public abstract RenderedPipelineDefinition<TOutput> Render(RenderArgs<TInput> renderArgs);
    ...
    public sealed class PipelineStagePipelineDefinition<TInput, TOutput> : PipelineDefinition<TInput, TOutput>
    {
        ...
        public override RenderedPipelineDefinition<TOutput> Render(RenderArgs<TInput> renderArgs)
        {
            ...
            IBsonSerializer currentSerializer = renderArgs.DocumentSerializer;
            foreach (var stage in _stages)
            {
                var renderedStage = stage.Render(currentSerializer, renderArgs.SerializerRegistry, renderArgs.LinqProvider);
            ...
            return new RenderedPipelineDefinition<TOutput>(
                pipeline,
                _outputSerializer ?? (currentSerializer as IBsonSerializer<TOutput>) ?? renderArgs.SerializerRegistry.GetSerializer<TOutput>());
        }
    ...
    internal class OptimizingPipelineDefinition<TInput, TOutput> : PipelineDefinition<TInput, TOutput>
    {
        ...
        public override RenderedPipelineDefinition<TOutput> Render(RenderArgs<TInput> renderArgs)
        {
            var rendered = _wrapped.Render(renderArgs);
