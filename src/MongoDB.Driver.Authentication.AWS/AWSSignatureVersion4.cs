...
namespace MongoDB.Driver.Authentication.AWS
{
    ...
    internal static class AWSSignatureVersion4
    {
        ...
        private static string GetSignature(string stringToSign, string secretAccessKey, string date, string region, string service)
        {
            var aws4SecretAccessKeyBytes = Encoding.ASCII.GetBytes($"AWS4{secretAccessKey}");
