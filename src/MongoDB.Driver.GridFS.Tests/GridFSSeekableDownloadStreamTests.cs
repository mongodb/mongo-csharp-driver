...
namespace MongoDB.Driver.GridFS.Tests
{
    ...
    public class GridFSSeekableDownloadStreamTests
    {
            ...
            [Values(false, true)] bool async)
        {
            ...
            var subject = bucket.OpenDownloadStreamAsync(id, options).GetAwaiter().GetResult();
            ...
            if (async)
            {
                var result = subject.ReadAsync(buffer, offset, count, CancellationToken.None).GetAwaiter().GetResult();
            ...
            [Values(false, true)] bool async)
        {
            ...
            if (async)
            {
                Action action = () => subject.ReadAsync(null, 0, 0).GetAwaiter().GetResult();
        ...
        public void Read_should_throw_when_count_is_invalid(int bufferLength, int offset, int count, bool async)
        {
            ...
            if (async)
            {
                Action action = () => subject.ReadAsync(buffer, offset, count, CancellationToken.None).GetAwaiter().GetResult();
        ...
        public void Read_should_throw_when_offset_is_invalid(int bufferLength, int offset, bool async)
        {
            ...
            if (async)
            {
                Action action = () => subject.ReadAsync(buffer, offset, 0, CancellationToken.None).GetAwaiter().GetResult();

                action.ShouldThrow<ArgumentOutOfRangeException>().And.ParamName.Should().Be("offset");
            }
            else
            {
                Action action = () => subject.Read(buffer, offset, 0);
