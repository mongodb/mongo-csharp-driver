...
namespace MongoDB.Bson.Tests.IO
{
    ...
    public class ByteArrayBufferTests
    {
        ...
        public void AccessBackingBytes_should_return_expected_result_for_length(int length, int expectedCount)
        {
            ...
            var subject = new ByteArrayBuffer(bytes);
        ...
        [TestCase(0, 64, 64)]
        [TestCase(0, 65, 128)]
        public void EnsureCapacity_should_have_expected_effect(int size, int capacity, int expectedCapacity)
        {
        ...
        [Test]
        public void EnsureCapacity_should_throw_when_capacity_is_invalid()
        {
            var subject = CreateSubject();

            Action action = () => subject.EnsureCapacity(-1);

            action.ShouldThrow<ArgumentException>().And.ParamName.Should().Be("capacity");
        }
        ...
        {
            var bytes = new byte[2];
            var subject = new ByteArrayBuffer(bytes, isReadOnly: true);
        ...
        [TestCase(2, 1)]
        [TestCase(2, 2)]
        public void Length_get_should_return_expected_result(int size, int length)
        {
            var subject = CreateSubject(size, length);
        ...
        [TestCase(2, 1)]
        [TestCase(2, 2)]
        public void Length_set_should_have_expected_effect(int size, int length)
        {
