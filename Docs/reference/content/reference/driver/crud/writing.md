...
```csharp
var newDoc = new BsonDocument("_id", 10).Add("x", 2);
var result = await collection.ReplaceOneAsync(
	filter: new BsonDocument("_id", 10),
	replacement: newDoc);
```

{{% note %}}`_id` values in MongoDB documents are immutable. If you specify an `_id` in the replacement document, it is important that the filter be on the `_id`.{{% /note %}}
