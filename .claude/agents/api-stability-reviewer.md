...
- Interface members added (breaks existing implementers). Default interface methods are **not** an option here: the driver multi-targets `net472;net6.0;net8.0;net10.0` (see `src/Directory.Build.props`), and `net472` cannot consume DIMs — adding any interface member is a hard break for every multi-target consumer.
