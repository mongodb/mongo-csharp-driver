Properties {
	...
	if($build_number -eq $null) {
		$build_number = [int]((Get-Date) - (Get-Date -Year 2000 -Month 1 -Day 1)).TotalDays
	}

	$version = "2.0.0.$build_number"
	$sem_version = "2.0.0-alpha$build_number"
...
Task Docs -precondition { BuildHasBeenRun } {
	...
	cp "$docs_dir\CSharpDriverDocs.chm" $chm_file
...
Task Installer -precondition { (BuildHasBeenRun) -and (DocsHasBeenRun) } {
	...
	Exec { msbuild "$installer_file" /t:Rebuild /p:Configuration=$config /p:Version=$version /p:SemVersion=$sem_version /p:ReleaseNotes=$release_notes_relative_path /p:License="License.rtf" /p:Documentation=$doc_relative_path /p:OutputPath=$artifacts_dir }
