...
namespace MongoDB.DriverUnitTests.Operations
{
    ...
    public class BulkWriteOperationTests
    {
        ...
        public void TestBatchSplittingBySizeWithErrorsOrdered()
        {
            ...
            var exception = Assert.Throws<MongoBulkWriteException>(() => { bulk.Execute(); });
        ...
        public void TestNonDefaultWriteConcern(bool ordered)
        {
            ...
            {
                Assert.Throws<MongoBulkWriteException>(() => { bulk.Execute(writeConcern); });
        ...
        public void TestW2AgainstStandalone(bool ordered)
        {
            if (_primary.InstanceType == MongoServerInstanceType.StandAlone)
            {
                ...
                {
                    var exception = Assert.Throws<MongoBulkWriteException>(() => { bulk.Execute(WriteConcern.W2); });
                    var result = exception.Result;
        ...
        public void TestWTimeoutPlusDuplicateKeyError()
        {
            ...
            using (Configuration.StopReplication(secondary))
            {
                ...
                var exception = Assert.Throws<MongoBulkWriteException>(() => bulk.Execute(new WriteConcern { W = all, WTimeout = TimeSpan.FromMilliseconds(1) }));
                var result = exception.Result;
