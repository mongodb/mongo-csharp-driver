...
{
    // Regression test for CSHARP-1559.
    [Collection(RegisterObjectSerializerFixture.CollectionName)]
    public class ImmutableClassConstructorMatchingTests
    {
        [Theory]
        ...
        public void Serialization_with_mismatching_type_between_constructor_argument_and_base_property()
        {
            ...
            e.Message.Should().Be("Creator map for class MongoDB.Bson.Tests.Serialization.ImmutableClassConstructorMatchingTests+DerivedWithMismatchedXTypeComparingWithBase has 2 arguments, but none are configured.");
        }
