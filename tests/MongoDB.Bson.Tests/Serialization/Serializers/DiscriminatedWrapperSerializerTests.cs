...
namespace MongoDB.Bson.Serialization.Serializers
{
    public class DiscriminatedWrapperSerializerTests
    {
        ...
        private static readonly IDiscriminatorConvention __discriminatorConvention2;
        private static readonly IBsonSerializer<C> __wrappedSerializer1;
        private static readonly IBsonSerializer<C> __wrappedSerializer2;

        static DiscriminatedWrapperSerializerTests()
        {
            __discriminatorConvention1 = new ScalarDiscriminatorConvention("_t");
            __discriminatorConvention2 = new ScalarDiscriminatorConvention("_u");
            var classMap1 = new BsonClassMap<C>(cm => { cm.AutoMap(); cm.MapProperty(c => c.X).SetElementName("x"); cm.Freeze(); });
            var classMap2 = new BsonClassMap<C>(cm => { cm.AutoMap(); cm.MapProperty(c => c.X).SetElementName("y"); cm.Freeze(); });
            __wrappedSerializer1 = new BsonClassMapSerializer<C>(classMap1);
            __wrappedSerializer2 = new BsonClassMapSerializer<C>(classMap2);
        }
