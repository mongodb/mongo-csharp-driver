...
namespace MongoDB.Bson.Tests.Serialization.Serializers
{
    public class ElementAppendingSerializerTests
    {
        ...
        private static readonly IBsonSerializer<C> __documentSerializer1;
        private static readonly IBsonSerializer<C> __documentSerializer2;
        private static readonly List<BsonElement> __elements1;
        private static readonly List<BsonElement> __elements2;
        private static readonly Action<BsonWriterSettings> __writerSettingsConfigurator1;
        private static readonly Action<BsonWriterSettings> __writerSettingsConfigurator2;

        static ElementAppendingSerializerTests()
        {
            var classMap1 = new BsonClassMap<C>(cm => { cm.AutoMap(); cm.MapProperty(x => x.X).SetElementName("x"); });
            var classMap2 = new BsonClassMap<C>(cm => { cm.AutoMap(); cm.MapProperty(x => x.X).SetElementName("y"); });
            __documentSerializer1 = new BsonClassMapSerializer<C>(classMap1.Freeze());
            __documentSerializer2 = new BsonClassMapSerializer<C>(classMap2.Freeze());
            __elements1 = new List<BsonElement> { new BsonElement("x", 1) };
            __elements2 = new List<BsonElement> { new BsonElement("x", 2) };
            __writerSettingsConfigurator1 = s => { };
            __writerSettingsConfigurator2 = s => { };
        }
