...
{
    // Regression test for CSHARP-4511.
    public class ObjectSerializerSeparateAllowedTypesTests
    {
        static ObjectSerializerSeparateAllowedTypesTests()
        {
        ...
        {
            var json = "{ Object : { _t : 'MongoDB.Bson.Tests.Serialization.Serializers.ObjectSerializerSeparateAllowedTypesTests+AllowedToBeDeserialized, MongoDB.Bson.Tests', X : 1  } }";
        ...
        {
            var json = "{ Object : { _t : 'MongoDB.Bson.Tests.Serialization.Serializers.ObjectSerializerSeparateAllowedTypesTests+NotAllowed, MongoDB.Bson.Tests', X : 1  } }";

            var exception = Record.Exception(() => BsonSerializer.Deserialize<C>(json));

            exception.Should().BeOfType<FormatException>();
            exception.Message.Should().Be("An error occurred while deserializing the Object property of class MongoDB.Bson.Tests.Serialization.Serializers.ObjectSerializerSeparateAllowedTypesTests+C: Type MongoDB.Bson.Tests.Serialization.Serializers.ObjectSerializerSeparateAllowedTypesTests+NotAllowed is not configured as a type that is allowed to be deserialized for this instance of ObjectSerializer.");
        }
