...
namespace MongoDB.Driver.Tests.Specifications.server_discovery_and_monitoring
{
    ...
    public class ServerDiscoveryAndMonitoringProseTests
    {
        ...
        public void Monitor_sleep_at_least_minHeartbeatFrequencyMS_between_checks()
        {
            ...
            var server = DriverTestConfiguration.Client.GetClusterInternal().SelectServer(new EndPointServerSelector(new DnsEndPoint(serverAddress.Host, serverAddress.Port)), OperationContext.NoTimeout);
        ...
        public void RoundTimeTrip_test()
        {
            ...
            using (var client = CreateClient(null, eventCapturer, heartbeatInterval, applicationName: "streamingRttTest"))
            {
                ...
                using (FailPoint.Configure(client.GetClusterInternal(), NoCoreSession.NewHandle(), failPointCommand))
                {
                    ...
                    var server = client.GetClusterInternal().SelectServer(WritableServerSelector.Instance, OperationContext.NoTimeout);
        ...
        public void ConnectionPool_cleared_on_failed_hello()
        {
            ...
            var failpointServer = DriverTestConfiguration.Client.GetClusterInternal().SelectServer(new EndPointServerSelector(new DnsEndPoint(serverAddress.Host, serverAddress.Port)), OperationContext.NoTimeout);
