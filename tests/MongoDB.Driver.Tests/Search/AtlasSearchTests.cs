...
namespace MongoDB.Driver.Tests.Search
{
    ...
    {
        private const string SearchWithVectorIndexName = "search-with-vector11";
        ...
        public AtlasSearchTests(ITestOutputHelper testOutputHelper) : base(testOutputHelper)
        {
            ...
            if (!TryGetIndex(moviesCollection, SearchWithVectorIndexName, out var _))
            {
                ...
                while (!TryGetIndex(moviesCollection, SearchWithVectorIndexName, out var indexDocument)
                       || !indexDocument["queryable"].AsBoolean)
                {
                    Thread.Sleep(5000);
                }
            }
        ...
        [Fact]
        public void SearchWithVectorSearch()
        {
            ...
            var vectorSearch = Builders<EmbeddedMovie>.Search.VectorSearch(x => x.Embedding, vector, limit: 5);

            var results = GetEmbeddedMoviesCollection<EmbeddedMovie>()
                .Aggregate()
        ...
        [Fact]
        public void SearchWithVectorSearch_Filter()
        {
            var expectedTitles = new[]
            {
                "Time Bandits",
            ...
            {
                Filter = Builders<EmbeddedMovie>.Search.Text(new[]{"fullplot", "title"}, "time"),
                NumberOfCandidates = 128
            };

            var vectorSearch = Builders<EmbeddedMovie>.Search.VectorSearch(x => x.Embedding, vector, limit: 5, options);
        ...
        [Fact]
        public void SearchWithVectorSearch_Exact()
        {
            var expectedTitles = new[]
            {
                "Willy Wonka & the Chocolate Factory",
        ...
        [Fact]
        public void SearchWithVectorSearch_ProjectAndScore()
        {
            var expectedTitles = new[]
            {
                "Time Bandits",
            ...
            var scoreBuilder = new SearchScoreDefinitionBuilder<EmbeddedMovie>();
            var vectorSearch = Builders<EmbeddedMovie>.Search.VectorSearch(x => x.Embedding, vector, limit: 5, options, scoreBuilder.Constant(1));
