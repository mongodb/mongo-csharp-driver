...
namespace MongoDB.Driver.Core.Operations
{
    public class DistinctOperationTests : OperationTestBase
    {
        ...
        public void CreateCommand_should_ignore_maxtime_if_timeout_specified(int timeoutMs)
        {
            ...
            var connectionDescription = OperationTestHelper.CreateConnectionDescription();
            using var session = OperationTestHelper.CreateSession();
            using var operationContext = new OperationContext(session, timeout: TimeSpan.FromMilliseconds(timeoutMs));
        ...
        public async Task Execute_should_set_operation_name([Values(false, true)] bool async)
        {
            ...
            await VerifyOperationNameIsSet(subject, async, "distinct");
        }
