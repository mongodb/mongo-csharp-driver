...
{
    public class FilterDefinitionBuilderNullableIntComparedToNullableIntWithStringRepresentationTests
    {
        private static RenderArgs<C> __renderArgs = new(BsonSerializer.SerializerRegistry.GetSerializer<C>(), BsonSerializer.SerializerRegistry);
        ...
        public void Eq_with_field_name_should_render_correctly(int? value, string expectedFilter)
        {
            ...
            filter.Render(__renderArgs).Should().Be(expectedFilter);
        }
