...
namespace MongoDB.Driver.Tests
{
    public class OperationExecutorTests
    {
        ...
        public async Task ExecuteReadOperationShouldStartAndDisposeImplicitSessionIfNeeded(bool shouldCreateSession, bool isAsync, IClientSessionHandle session)
        {
            ...
            _ = isAsync ?
                await subject.ExecuteReadOperationAsync(readOperation, readOperationOptions, session, CancellationToken.None) :
                subject.ExecuteReadOperation(readOperation, readOperationOptions, session, CancellationToken.None);
        ...
        public async Task ExecuteWriteOperationShouldStartAndDisposeImplicitSessionIfNeeded(bool shouldCreateSession, bool isAsync, IClientSessionHandle session)
        {
            ...
            _ = isAsync ?
                await subject.ExecuteWriteOperationAsync(writeOperation, writeOperationOptions, session, CancellationToken.None) :
                subject.ExecuteWriteOperation(writeOperation, writeOperationOptions, session, CancellationToken.None);
