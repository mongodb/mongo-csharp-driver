...
public class MqlHashTests
{
    [Fact]
    public void SerializerFinder_should_resolve_mql_hash()
    {
        var expression = TestHelpers.MakeLambda<MyModel, BsonBinaryData>(model => Mql.Hash(model.Bytes, MqlHashAlgorithm.SHA256));
        ...
        serializerMap.GetSerializer(expression.Body).Should().BeOfType<BsonBinaryDataSerializer>();
    }
