...
public class MqlHexHashTests
{
    [Fact]
    public void SerializerFinder_should_resolve_mql_hexhash()
    {
        var expression = TestHelpers.MakeLambda<MyModel, string>(model => Mql.HexHash(model.Bytes, MqlHashAlgorithm.SHA256));
        ...
        serializerMap.GetSerializer(expression.Body).Should().BeOfType<StringSerializer>();
    }
