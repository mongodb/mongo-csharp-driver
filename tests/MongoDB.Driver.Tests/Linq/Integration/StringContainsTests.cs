...
public class StringContainsTests : LinqIntegrationTest<StringContainsTests.ClassFixture>
{
    public StringContainsTests(ClassFixture fixture)
        : base(fixture)
    {
    }
    ...
    public void Where_with_Contains_string_should_work(string value)
    {
        var collection = Fixture.Collection;

        var results = collection.AsQueryable().Where(x => x.Str.Contains(value)).ToList();

        AssertTestCase(results, s => s.Contains(value, StringComparison.Ordinal));
    }

    [Theory]
    [MemberData(nameof(TestChars))]
    public void Where_with_Contains_char_should_work(char value)
    {
        var collection = Fixture.Collection;

        var results = collection.AsQueryable().Where(x => x.Str.Contains(value)).ToList();

        AssertTestCase(results, s => s.Contains(value));
    }
#endif
    ...
    public sealed class ClassFixture : MongoCollectionFixture<C>
    {
        ...
        private static C[] CreateDocuments()
        {
            ...
            {
                documents.Add(new C { Id = id++, Str = s });             // Contains(s)
                documents.Add(new C { Id = id++, Str = s + "abc" });     // Contains(s)
                documents.Add(new C { Id = id++, Str = "abc" + s });     // Contains(s)
                documents.Add(new C { Id = id++, Str = "x" + s + "y" }); // Contains(s)
            }

            foreach (var c in TestChars)
            {
                documents.Add(new C { Id = id++, Str = $"abc{c}def" });  // Contains(c)
            }
