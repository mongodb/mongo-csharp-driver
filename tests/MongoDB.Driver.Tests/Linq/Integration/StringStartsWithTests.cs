...
public class StringStartsWithTests : LinqIntegrationTest<StringStartsWithTests.ClassFixture>
{
    public StringStartsWithTests(ClassFixture fixture)
        : base(fixture)
    {
    }
    ...
    public void Where_with_StartsWith_string_should_work(string value)
    {
        var collection = Fixture.Collection;

        var results = collection.AsQueryable().Where(x => x.Str.StartsWith(value)).ToList();

        AssertTestCase(results, s => s.StartsWith(value, StringComparison.Ordinal));
    }

#if !NET472_OR_GREATER
    [Theory]
    [MemberData(nameof(TestChars))]
    public void Where_with_StartsWith_char_should_work(char value)
    {
        var collection = Fixture.Collection;

        var results = collection.AsQueryable().Where(x => x.Str.StartsWith(value)).ToList();

        AssertTestCase(results, s => s.Length > 0 && s[0] == value);
    }
#endif
    ...
    public sealed class ClassFixture : MongoCollectionFixture<C>
    {
        ...
        private static C[] CreateDocuments()
        {
            ...
            {
                documents.Add(new C { Id = id++, Str = s });             // StartsWith(s)
                documents.Add(new C { Id = id++, Str = s + "abc" });     // StartsWith(s)
            ...
            {
                documents.Add(new C { Id = id++, Str = $"{c}abc" });     // StartsWith(c)
                documents.Add(new C { Id = id++, Str = $"abc{c}" });
            }
