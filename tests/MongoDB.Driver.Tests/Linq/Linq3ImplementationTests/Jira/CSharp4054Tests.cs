...
namespace MongoDB.Driver.Tests.Linq.Linq3ImplementationTests.Jira
{
    public class CSharp4054Tests : Linq3IntegrationTest
    {
        ...
        public void Join_should_work([Values(LinqProvider.V2, LinqProvider.V3)] LinqProvider linqProvider)
        {
            var people = CreatePeopleCollection(linqProvider);
            var movies = CreateMoviesCollection(linqProvider);
            ...
            {
                var exception = Record.Exception(() => Translate(people, queryable));
        ...
        private IMongoCollection<Person> CreatePeopleCollection(LinqProvider linqProvider)
        {
            var collection = GetCollection<Person>("people", linqProvider);
            CreateCollection(collection, __person1, __person2, __person3);
            return collection;
        }

        private IMongoCollection<Movie> CreateMoviesCollection(LinqProvider linqProvider)
        {
            var collection = GetCollection<Movie>("movies", linqProvider);
