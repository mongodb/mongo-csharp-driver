...
namespace MongoDB.Driver.Tests.Linq.Linq2ImplementationTestsOnLinq3.Translators
{
    public class FindProjectionTranslatorTests
    {
        ...
        private ProjectedResult<TProjection> Project<TProjection>(Expression<Func<Root, TProjection>> expression, string json)
        {
            var context = new TranslationContext();
            var findProjection = ExpressionToFindProjectionTranslator.Translate<TProjection>(context, expression);

            var projectionDocument = findProjection.Render().AsBsonDocument;
            var projectedValue = DeserializeProjectedValue(findProjection.ProjectionSerializer, json);

            return new ProjectedResult<TProjection> { Projection = projectionDocument, Value = projectedValue };

            static TProjection DeserializeProjectedValue(IBsonSerializer<TProjection> serializer, string json)
            {
