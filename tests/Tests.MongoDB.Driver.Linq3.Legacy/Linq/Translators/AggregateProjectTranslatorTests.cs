...
namespace Tests.MongoDB.Driver.Linq3.Legacy.Translators
{
    public class AggregateProjectTranslatorTests : IntegrationTestBase
    {
        ...
        public void Should_translate_anyElementTrue_with_predicate()
        {
            ...
            result.Projection.Should().Be("{ Result : { $reduce : { input : '$G', initialValue : false, in : { $cond : { if : '$$value', then : true, else : { $gt : ['$$this.E.F', 40] } } } } }, _id : 0 }");
        ...
        public void Should_translate_arrayElemAt_using_a_variable_ElementAt_followed_by_a_field()
        {
            ...
            result.Projection.Should().Be("{ Result: { $let: { vars: { this: { \"$arrayElemAt\": [\"$G\", \"$T.one\"] } }, in: \"$$this.D\" } }, _id: 0 }");
        ...
        public void Should_translate_condition()
        {
            ...
            result.Projection.Should().Be("{ Result: { \"$cond\": { if  : { \"$eq\": [\"$A\", \"funny\"] }, then : \"a\", else : \"b\" } }, _id: 0 }");
